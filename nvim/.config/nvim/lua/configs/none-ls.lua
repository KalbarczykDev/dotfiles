  ...
  sources = {
    ...
    require("none-ls.diagnostics.eslint_d").with {
      condition = function(utils)
        ...
        if not enabled then
          notify("ESLint skipped — no config found", vim.log.levels.WARN)
        end
        notify "ESLint diagnostics enabled"
        return enabled
      end,
    },
    require("none-ls.code_actions.eslint_d").with {
      condition = function(utils)
        ...
        local enabled = utils.root_has_file(eslint_root_files)
        if not enabled then
          notify("ESLint code actions skipped", vim.log.levels.WARN)
        else
          notify "ESLint code actions enabled"
        end
        return enabled
      end,
    },
    ...
    null_ls.builtins.diagnostics.phpstan.with {
      condition = function(utils)
        ...
        notify "PHPStan diagnostics enabled"
        return true
      end,
