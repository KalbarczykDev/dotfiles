...
update_track() {
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME background.color=0xfffab387
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME background.color=0xfffab387 label.drawing=no
    else
        sketchybar --set $NAME background.color=0xffa6e3a1
    fi
}
